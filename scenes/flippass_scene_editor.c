...
static void flippass_editor_otp_type_change_callback(VariableItem* item) {
    ...
    variable_item_set_current_value_text(item, flippass_editor_otp_type_text(app->editor_otp_kind));
...
static void flippass_editor_otp_time_zone_change_callback(VariableItem* item) {
    ...
    app->editor_otp_time_zone_hours = (int8_t)((int16_t)index - 12);
    snprintf(
        text,
        sizeof(text),
        "%+ldh",
        (long)app->editor_otp_time_zone_hours);
    variable_item_set_current_value_text(item, text);
}
...
static void flippass_editor_add_otp_encoding_item(App* app) {
    VariableItem* item = variable_item_list_add(
        ...
        "Encoding",
        4U,
        flippass_editor_otp_encoding_change_callback,
        app);
...
static void flippass_editor_add_otp_period_item(App* app) {
    VariableItem* item = variable_item_list_add(
        ...
        "Period",
        255U,
        flippass_editor_otp_period_change_callback,
        app);
...
static void flippass_editor_add_otp_time_zone_item(App* app) {
    VariableItem* item = variable_item_list_add(
        ...
        app);
    int8_t hours = app->editor_otp_time_zone_hours;
    if(hours < -12 || hours > 14) {
        hours = 0;
    }
    variable_item_set_current_value_index(item, (uint8_t)(hours + 12));
...
static bool flippass_editor_parse_algorithm_text(
    const char* text,
    FlipPassOtpAlgorithm* out_algorithm) {
    if(text == NULL || text[0] == '\0') {
        return false;
    }
...
static void flippass_editor_set_otp_defaults(App* app) {
    ...
    app->editor_otp_digits = FLIPPASS_OTP_DEFAULT_DIGITS;
    app->editor_otp_period = FLIPPASS_OTP_DEFAULT_PERIOD;
    app->editor_otp_time_zone_hours = app->otp_time_zone_hours;
...
static bool flippass_editor_load_otp_field_text(
    App* app,
    KDBXCustomField* field,
    FuriString* error) {
...
static bool flippass_editor_load_otp_entry_secret(
    App* app,
...
static bool flippass_editor_load_otp_from_entry(App* app, FuriString* error) {
    ...
    KDBXCustomField* counter = flippass_editor_find_custom_field(app->editor_entry, "HmacOtp-Counter");
    if(counter != NULL) {
    ...
    KDBXCustomField* length = flippass_editor_find_custom_field(app->editor_entry, "TimeOtp-Length");
    if(length != NULL) {
        uint64_t digits = 0ULL;
    ...
    KDBXCustomField* period = flippass_editor_find_custom_field(app->editor_entry, "TimeOtp-Period");
    if(period != NULL) {
        uint64_t period_value = 0ULL;
    ...
    if(algorithm != NULL) {
        ...
        if(algorithm->value != NULL && flippass_editor_parse_algorithm_text(algorithm->value, &parsed)) {
            app->editor_otp_algorithm = parsed;
        }
    }
...
static bool flippass_editor_delete_otp_entry_field_by_name(
    App* app,
    const char* name,
    FuriString* error) {
    bool ok = true;
...
static bool flippass_editor_execute_otp_delete(App* app) {
    ...
    if(ok) {
        ...
        scene_manager_search_and_switch_to_previous_scene(app->scene_manager, FlipPassScene_Editor);
    } else {
        flippass_scene_status_show(
...
static bool flippass_editor_execute_otp_save(App* app) {
    ...
    if(app->editor_otp_kind == FlipPassOtpKindHmac) {
        if(counter != FLIPPASS_OTP_DEFAULT_COUNTER) {
            ...
            ok = flippass_editor_save_otp_field(
                app, "HmacOtp-Counter", text, false, error);
        }
    } else {
    ...
    if(ok) {
        app->otp_time_zone_hours = app->editor_otp_time_zone_hours;
...
static bool flippass_editor_execute_new_database_commit(App* app) {
    ...
    ok = flippass_db_create_new_database(
        app,
        furi_string_get_cstr(root_name),
        requested_cipher,
    ...
    if(ok) {
        ...
        ok = flippass_editor_save_database(
            app,
            furi_string_get_cstr(target_path),
...
static bool flippass_editor_crud_host_delete_group(
    void* context,
    KDBXGroup* group,
    FuriString* error) {
...
static bool flippass_editor_crud_host_delete_entry(
    void* context,
    KDBXEntry* entry,
    FuriString* error) {
...
static bool flippass_editor_execute_delete(App* app, FlipPassEditorDialogState state) {
    FuriString* load_error = furi_string_alloc();
    const FlipPassEditorCrudPluginV1* plugin =
        flippass_editor_crud_plugin_load(app, load_error);
...
static void flippass_editor_handle_enter(App* app, uint32_t index) {
    ...
    case FlipPassEditorModeEditOtp:
        ...
        } else if(app->editor_otp_kind == FlipPassOtpKindHmac && index == 3U) {
...
static void flippass_editor_cancel(App* app) {
    ...
    if(mode == FlipPassEditorModeEditOtp) {
        flippass_editor_restore_after_otp(app);
        if(!scene_manager_search_and_switch_to_previous_scene(app->scene_manager, FlipPassScene_Editor)) {
            scene_manager_previous_scene(app->scene_manager);
        }
        return;
    }
